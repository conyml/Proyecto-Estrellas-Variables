...
paths='/Users/Carolina/Desktop/Pruebas/'
...
for i in range(len(files)):
	...
	nn2=np.argwhere(np.isnan(np.modf((np.array(algo['HJD'])-algo['HJD'][np.argmax(algo['MAG'])])/7.158278))[0])
	magnitudes=list(np.delete(algo['MAG'],nn2))
	JD=np.delete(algo['HJD'],nn2)
	phasetotal_plit=np.modf((np.array(JD)-JD[np.argmax(magnitudes)])/7.158278)[0]
	...
	if len(phasetotal_plit)>0:
		...
		periods.append(7.157278)
		names_var.append('OGLE-LMC-CEP-0103')

def lightcurve(num,magband,colordot,maglist,phase_ogle,var_name,periods):
	with PdfPages('/Users/Carolina/Desktop/Lightcurves_Dasch_AllEpoch.pdf') as pdf:
		i = 0
		while i < len(maglist):
			fig=plt.figure(i)
			j = 0
