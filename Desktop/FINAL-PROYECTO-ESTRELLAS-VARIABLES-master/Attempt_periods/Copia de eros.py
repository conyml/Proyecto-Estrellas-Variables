...
for i in range(len(files)):
	algo=pd.read_csv(files[i],
		header=None, delimiter=',', names=['HJD','MAG','BAND','r'])
	#algo['HJD'] = np.where( algo['HJD'] > 10000.0, algo['HJD']+2450000.0, algo['HJD'])
	...
	nn2=np.argwhere(np.isnan(np.modf((np.array(algo['HJD'])-algo['HJD'][np.argmax(algo['MAG'])])/28.3738735))[0])
	magnitudes=list(np.delete(algo['MAG'],nn2))
	JD=np.delete(algo['HJD'],nn2)
	phasetotal_plit=np.modf((np.array(JD)-JD[np.argmax(magnitudes)])/28.3738735)[0]
	...
	if len(phasetotal_plit)>0:
		...
		periods.append(28.3738735)
		names_var.append('OGLE-LMC-CEP-0068')

def lightcurve(num,magband,colordot,maglist,phase_ogle,var_name,periods):
	with PdfPages('/Users/Carolina/Desktop/OGLE-LMC-CEP-0068_normed_dasch.pdf') as pdf:
		i = 0
		while i < len(maglist):
			fig=plt.figure(i)
			j = 0
...
lightcurve_V=lightcurve(4,'B','red',Imags,Iphases,names_var,periods)
